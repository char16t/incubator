...
class UserApiTest extends BaseTest with TestEmbeddedPostgres with Eventually {
  ...
  "/user/join" should "not register if data is invalid" in {
    // given
  ...
  "/user/join" should "not register if email is taken" in {
    // given
    val (login, email, password) = randomLoginEmailPassword()
  ...
  "/user/join" should "send a welcome email" in {
    // when
    val RegisteredUser(login, email, _, _) = newRegisteredUsed()

    // then
    modules.emailService.sendBatch().unwrap
    DummyEmailSender.findSentEmail(email, s"подтверждение регистрации для пользователя $login") shouldBe 'defined
  }
