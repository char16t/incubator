...
describe('behaviour', () => {
  it('an error should appear under empty login input on blur', () => {
    ...
    expect(wrapper.contains(<p className="validation-message" key={0}>как минимум 3 символа!</p>)).toBe(true);
  });
  ...
  it('an error should appear under empty email input on blur', () => {
    ...
    expect(wrapper.contains(<p className="validation-message" key={0}>скорее всего, такого адреса электронной почты не существует</p>)).toBe(true);
  });
