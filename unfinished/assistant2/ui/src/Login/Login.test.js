...
describe('behaviour', () => {
  ...
  it('an error should appear under empty login input on blur', () => {
    ...
    expect(wrapper.contains(<p className="validation-message" key={0}>логин/почта не может быть пустой!</p>)).toBe(true);
  });
  ...
  it('an error should appear under empty password input on blur', () => {
    ...
    expect(wrapper.contains(<p className="validation-message" key={0}>пароль не может быть пустым!</p>)).toBe(true);
  });
